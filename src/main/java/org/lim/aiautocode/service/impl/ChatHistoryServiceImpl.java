...
import org.lim.aiautocode.mapper.ChatHistoryMapper;
...
public class ChatHistoryServiceImpl extends ServiceImpl<ChatHistoryMapper, ChatHistory>  implements ChatHistoryService{
    ...
    public boolean addChatMessage(Long appId, String message, String messageType, Long userId) {
        // 参数验证 - 使用更简洁的验证方式
        validateAddChatMessageParams(appId, message, messageType, userId);
        ...
        ChatHistory chatHistory = ChatHistory.builder()
                ...
                .userId(userId)
                .createTime(LocalDateTime.now()) // 添加创建时间
                .build();

        try {
            return this.save(chatHistory);
        } catch (Exception e) {
            log.error("保存聊天记录失败, appId: {}, userId: {}", appId, userId, e);
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "保存聊天记录失败");
        }
    }
     ...
     * 验证添加聊天消息的参数
     */
    private void validateAddChatMessageParams(Long appId, String message, String messageType, Long userId) {
        ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "应用ID不能为空或无效");
